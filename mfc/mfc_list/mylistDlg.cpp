...
CMylistDlg::CMylistDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_MFC_LIST_DIALOG, pParent)
{
...
BOOL CMylistDlg::OnInitDialog()
{
	...
	int nWidth = rc.Width();
	m_ListCtrl.InsertColumn(0, L"姓名", 0, nWidth / 3);
	m_ListCtrl.InsertColumn(1, L"性别", 0, nWidth / 3);
	m_ListCtrl.InsertColumn(2, L"年龄", 0, nWidth / 3);
	//插入数据
	m_ListCtrl.InsertItem(0, L"张三");
	m_ListCtrl.SetItemText(0, 1, L"男");
	m_ListCtrl.SetItemText(0, 2, L"20");
